...
TOOLS = {
    ...
    "show_sentry_connections": _read_tool(
        "show_sentry_connections",
        "List sanitized Sentry connections for a workspace, including topology and project-catalog summaries.",
        lambda args: list_sentry_connections(args["workspacePath"]),
    ),
    "connect_sentry": _write_tool(
        "connect_sentry",
        "Create or validate a workspace-scoped Sentry connection using a bearer token and auto-discovered topology.",
        lambda args: connect_sentry(
            args["workspacePath"],
            base_url=args["baseUrl"],
            token=args["token"],
            organization_slug=args.get("organizationSlug"),
            label=args.get("label"),
            connection_id=args.get("connectionId"),
            project_scope=args.get("projectScope"),
            default=args.get("default", False),
    ...
    "update_sentry_connection": _write_tool(
        "update_sentry_connection",
        "Update a Sentry connection, optionally rotate its token, and refresh the cached topology.",
        lambda args: update_sentry_connection(
            args["workspacePath"],
            args["connectionId"],
            base_url=args.get("baseUrl"),
            token=args.get("token"),
    ...
    "remove_sentry_connection": _write_tool(
        "remove_sentry_connection",
        "Remove a Sentry connection and its cached topology metadata from a workspace.",
        lambda args: remove_sentry_connection(args["workspacePath"], args["connectionId"]),
        {
            "workspacePath": {"type": "string"},
            "connectionId": {"type": "string"},
        },
    ...
    "test_sentry_connection": _write_tool(
        "test_sentry_connection",
        "Re-run connection validation and refresh the cached Sentry project topology.",
        lambda args: test_sentry_connection(args["workspacePath"], args["connectionId"]),
        {
            "workspacePath": {"type": "string"},
            "connectionId": {"type": "string"},
        },
    ...
    "search_sentry_issues": _write_tool(
        "search_sentry_issues",
        "Run a Sentry issue search, persist the shortlist, and use saved topology to narrow candidate projects.",
        lambda args: search_sentry_issues(
            args["workspacePath"],
            query_text=args["queryText"],
            connection_id=args.get("connectionId"),
            external_issue=args.get("externalIssue"),
            task_id=args.get("taskId"),
            project_slugs=args.get("projectSlugs"),
            environment=args.get("environment"),
            stats_period=args.get("statsPeriod", "14d"),
            limit=args.get("limit", 8),
            shortlist_size=args.get("shortlistSize", 3),
        ),
        {
            "workspacePath": {"type": "string"},
            "queryText": {"type": "string"},
            ...
            "externalIssue": {"type": "object"},
            "taskId": {"type": "string"},
            "projectSlugs": {"type": "array", "items": {"type": "string"}},
    ...
    "show_sentry_issue_queue": _read_tool(
        "show_sentry_issue_queue",
        "Read the current or explicit persisted Sentry search session and shortlist queue.",
        lambda args: show_sentry_issue_queue(args["workspacePath"], search_session_id=args.get("searchSessionId")),
        {"searchSessionId": {"type": "string"}},
    ),
    "collect_sentry_context": _write_tool(
        "collect_sentry_context",
        "Collect a rich Sentry diagnostic packet with issue details, event stack traces, device contexts, and tag breakdowns.",
        lambda args: collect_sentry_context(
            args["workspacePath"],
            query_text=args.get("queryText"),
            connection_id=args.get("connectionId"),
            external_issue=args.get("externalIssue"),
